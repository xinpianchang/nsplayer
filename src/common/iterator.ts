...
type NativeIteratorResult<T, TReturn = any> =
  | NativeIteratorYieldResult<T>
  | NativeIteratorReturnResult<TReturn>
...
export namespace Iterator {
  ...
  export function fromArray<T>(array: readonly T[], index = 0, length = array.length): Iterator<T> {
    return {
      next(): IteratorResult<T> {
        if (index >= length) {
          return FIN
...
export class ArrayIterator<T> implements INextIterator<T> {
  ...
  constructor(items: readonly T[], start = 0, end: number = items.length, index = start - 1) {
    this.items = items
    this.start = start
    this.end = end
    this.index = index
